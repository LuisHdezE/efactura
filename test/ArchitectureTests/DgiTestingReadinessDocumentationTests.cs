...
public sealed class DgiTestingReadinessDocumentationTests
{
    ...
    public void Current_checkpoint_and_testing_reconciliation_keep_the_formal_DGI_gate_fail_closed()
    {
        ...
        Assert.Contains(
            "main@b2ea590779d229d8a2eab51111562a0447c6b52e",
            currentState,
        ...
        Assert.Contains(
            ...
            StringComparison.Ordinal);
        Assert.Contains(
            "Current pending governed increment: PR #84",
            currentState,
            StringComparison.Ordinal);
        Assert.Contains(
        ...
        Assert.Contains(
            ...
            StringComparison.Ordinal);
        Assert.Contains(
            "Append-only observation of DGI Reporte Diario later states `DR`, `ER` and `FR`",
            currentState,
            StringComparison.Ordinal);
        Assert.Contains(
            "Read-only reconciliation policy mapping `DR -> Consistent`",
            currentState,
            StringComparison.Ordinal);
        Assert.Contains(
            "Deterministic local Sobre v05 packaging",
            currentState,
            StringComparison.Ordinal);
        Assert.Contains(
