# UI-PROCUREMENT-001 v1-responsive-suite — Approved visual working baseline
...
The fingerprint above identifies the exact generated visual explicitly approved by Luis. No regenerated or visually similar image may silently replace this baseline under the same version.

## Owner approval

Luis explicitly approved this exact baseline with:

> Apruebo UI-PROCUREMENT-001 v1-responsive-suite como baseline visual

This approval promotes the exact candidate previously recorded under `documentation/ui/references/drafts/UI-PROCUREMENT-001/v1-responsive-suite/`.

## Approved coverage

The approved board contains the coordinated responsive suite for:
...
The baseline preserves the established eFactura visual language and governs the visual direction for the `Compras` workspace, including:
...
Visual approval does not prove executable procurement capability.

All purchase-order, supplier, quantity, amount, status, date and receipt values shown in the board remain illustrative visual fixtures until executable backend contracts exist and a fresh reconciliation authorizes implementation.

The baseline does not authorize live behavior for:
...
This approved baseline must be interpreted together with:
...
- visual approval authorizes preservation and implementation fidelity only when the execution gate is later satisfied;
...
The exact PNG is cryptographically fingerprinted above. The current repository lane has not inserted the binary bytes into Git, therefore:
