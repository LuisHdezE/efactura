...
const nestedEfaturaAssetRefs = assetRefs.filter((ref) => ref.startsWith('/efactura/assets/'));

if (rootAssetRefs.length === 0) {
  throw new Error('Deploy routing guard failed: no root-scoped /assets/ references were emitted for the efactura subdomain.');
}

if (nestedEfaturaAssetRefs.length > 0) {
  throw new Error(`Deploy routing guard failed: stale /efactura/assets/ references remain: ${nestedEfaturaAssetRefs.join(', ')}`);
}
...
console.log(`Deploy routing guard PASS: ${rootAssetRefs.length} asset references use root /assets/ and BrowserRouter follows BASE_URL.`);