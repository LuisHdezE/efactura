...
public sealed class TaxProfilesController : ControllerBase
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var businessDate = effectiveOn ?? DateOnly.FromDateTime(DateTime.UtcNow);
        var result = await _list.ExecuteAsync(
            new TaxProfileSearchRequest(organizationId, businessDate, search, activeOnly, page, pageSize),
            cancellationToken);
    ...
    private static TaxProfileDto Map(TaxProfileView profile) => new(
        profile.Id.ToString(),
        profile.Version,
        profile.Code,
        profile.Name,
